...
)
{
	...
	if((attr & AVR_FS_FILE_ATTR_TYPE_gm) != AVR_FS_FILE_ATTR_TYPE_LINK && (attr & AVR_FS_FILE_ATTR_TYPE_gm) != AVR_FS_FILE_ATTR_TYPE_STRUCT)/* If is a link no new memory is allocated. */
	{
	...
	avr_fs *cur_dj_search_entry = actual_dir + 1;/* Skip write actual dir address to the first entry of current directory, because point to parent directory. */
	...
	if(!empty_entry_discovered)
	{
		...
		for(; search_drive < RAM_FS_DRIVES_MAX; search_drive++)
		{
			if(*dj == ram_fs_root[search_drive])
			{
				...
				//*dj = actual_dir;/* The root directory has been relocated, actualize the return pointer. */
		...
		//avr_fs *pointed_dj = (avr_fs *)actual_dir->data + (actual_dir->parent_entry_offset / sizeof(avr_fs));/* Write the new address of actual directory to entry on parent directory. */
		//pointed_dj->data = (unsigned char *)pointed_dj;
	...
	if((attr & AVR_FS_FILE_ATTR_TYPE_gm) == AVR_FS_FILE_ATTR_TYPE_DIR || (attr & AVR_FS_FILE_ATTR_TYPE_gm) == AVR_FS_FILE_ATTR_TYPE_LINK)
	{
			...
			if(strlen(segment_start))
			{
				...
				else
				{
					if(mode == FA_CREATE_ALWAYS || mode == FA_OPEN_ALWAYS)
					{
						...
						{
							if((cur_dj->file_attr & AVR_FS_FILE_ATTR_TYPE_gm) != AVR_FS_FILE_ATTR_TYPE_DIR)/* If a fs is available, this will be removed. */
							{
#if (RAM_FS_DEBUG == 1)
...
FRESULT fs_init(char *fs_name, int drive)
{
	...
	ram_fs->file_attr = AVR_FS_FILE_ATTR_TYPE_DIR;
